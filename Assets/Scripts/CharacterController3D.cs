    ...
    public class CharacterController3D : MonoBehaviour
    {
        ...
        void moveHorizontally(ref Vector3 deltaMovement)
        {
            ...
            for (var i = 0; i < totalHorizontalRays; i++)
            {
                ...
                if (i == 0 && collisionState.wasGroundedLastFrame)
                    _hit = Physics.Raycast(ray, rayDirection, out _raycastHit, rayDistance, platformMask);
                else
                    _hit = Physics.Raycast(ray, rayDirection, out _raycastHit, rayDistance, platformMask & ~oneWayPlatformMask);
            ...
            if (angle < slopeLimit)
            {
                ...
                if (deltaMovement.y < jumpingThreshold)
                {
                    ...
                    if (collisionState.wasGroundedLastFrame)
                        hit = Physics.Raycast(ray, deltaMovement.normalized, out raycastHit, deltaMovement.magnitude, platformMask);
                    else
                        hit = Physics.Raycast(ray, deltaMovement.normalized, out raycastHit, deltaMovement.magnitude, platformMask & ~oneWayPlatformMask);
        ...
        void moveVertically(ref Vector3 deltaMovement)
        {
            ...
            for (var i = 0; i < totalVerticalRays; i++)
            {
                ...
                DrawRay(ray, rayDirection * rayDistance, Color.red);
                _hit = Physics.Raycast(ray, rayDirection, out _raycastHit, rayDistance, mask);
                if (_hit)
                {
                    // set our new deltaMovement and recalculate the rayDistance taking it into account
        ...
        private void handleVerticalSlope(ref Vector3 deltaMovement)
        {
            ...
            _hit = Physics.Raycast(slopeRay, rayDirection, out _raycastHit, slopeCheckRayDistance, platformMask);
            if (_hit)
            {
